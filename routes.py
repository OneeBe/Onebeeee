...
from models import Player, Quest, PlayerQuest, Achievement, PlayerAchievement, CustomTitle, PlayerTitle, GradientTheme, PlayerGradientSetting, SiteTheme, ShopItem, ShopPurchase, Clan, ClanMember, Tournament, TournamentParticipant, PlayerActiveBooster, AdminCustomRole, PlayerAdminRole
import os
import csv
import io
...
def change_language(language):
    ...
    if language in supported_languages:
        ...
        flash(f'Язык изменен на {language}!', 'success')
...
def admin_create_role():
    ...
    try:
        ...
        import magic # Ensure python-magic is installed: pip install python-magic
        ...
        if 'emoji_file' in request.files:
            ...
            if file and file.filename:
                ...
                allowed_mimes = {'image/png', 'image/svg+xml', 'image/webp', 'image/gif'}
                ...
                max_size_text = '512KB' if file_ext == 'gif' else '200KB'

                if file_size > max_size:
                    flash(f'Файл слишком большой! Максимум {max_size_text}', 'error')
                    return redirect(url_for('admin_roles'))

                # Check MIME type
                try:
                    # Ensure magic library is installed: pip install python-magic
                    mime_checker = magic.Magic(mime=True)
                    mime_type = mime_checker.from_buffer(file.read(1024))
                    ...
                    emoji_is_animated = mime_type in ['image/gif', 'image/webp']
                    
                except Exception as e:
                    app.logger.warning(f"Could not check MIME type (magic library might be missing or file is empty): {e}")
                    # If magic is not available, assume animated for gif files
                    emoji_is_animated = file_ext in ['gif', 'webp']
                    file.seek(0) # Ensure file pointer is reset

                # For GIF files, verify they are actually animated (basic check)
                if file_ext == 'gif':
                    try:
                        ...
                        if not getattr(img, 'is_animated', False):
                            flash('GIF файл должен быть анимированным!', 'error')
                            return redirect(url_for('admin_roles'))
                        file.seek(0)
                    except Exception as e:
                        app.logger.warning(f"Could not verify GIF animation: {e}")
                        # Continue anyway if PIL is not available

                # Create emojis directory if it doesn't exist
                emojis_dir = os.path.join('static', 'emojis')
                os.makedirs(emojis_dir, exist_ok=True)

                # Generate unique filename with UUID
                unique_filename = f"{uuid.uuid4().hex}.{file_ext}"
                file_path = os.path.join(emojis_dir, unique_filename)

                try:
                    file.save(file_path)
                    emoji_url = f"/static/emojis/{unique_filename}"
                except Exception as e:
                    app.logger.error(f"Error saving emoji file: {e}")
                    flash('Ошибка при сохранении файла эмодзи!', 'error')
                    return redirect(url_for('admin_roles'))

        role = AdminCustomRole(
            name=name,
            color=color,
            emoji=emoji if emoji and not emoji_url and not emoji_class else None,
            emoji_url=emoji_url,
            emoji_class=emoji_class if emoji_class and not emoji_url else None,
...
def admin_assign_role():
    ...
    try:
        ...
        if not player_nickname or not role_id:
            flash('Выберите игрока и роль!', 'error')
            return redirect(url_for('admin_roles'))

        player = Player.query.filter_by(nickname=player_nickname).first()
        role = AdminCustomRole.query.get(role_id)
...
def admin_delete_role(role_id):
    ...
    try:
        ...
        # Deactivate all player assignments
        PlayerAdminRole.query.filter_by(role_id=role_id).update({'is_active': False})
