...
with app.app_context():
    ...
    try:
        # Test if database schema is up to date by trying to query a Player
        Player.query.first()
        ...
        app.logger.error(f"Database schema error detected: {e}")
        # If there's an error (likely schema mismatch), recreate tables
        try:
            app.logger.info("Recreating database schema...")
            db.drop_all()
            db.create_all()
            ...
            app.logger.info("Database schema recreated successfully!")
